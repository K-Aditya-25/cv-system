...
from .linkedin import discovery_query, linkedin_job_id, rank_candidates
...
def resolve_linkedin(service, request) -> ResolutionResult:
    ...
    _log(f"request_id={request.request_id} source=telegram_url strategy=linkedin_fast_pipeline "
         f"url={request.url}")
    result = _attempt(service, request.url, "direct_http")
    if result:
        return result
    if service.discover:
        ...
        if ranked:
            result = _attempt(service, ranked[0], "discovered_top")
            if result:
                return result
...
def _attempt(service, url: str, pathway: str) -> ResolutionResult | None:
    ...
    try:
        ...
        job = extract_linkedin_fast(page.body.decode(_charset(page.content_type), errors="replace"))
